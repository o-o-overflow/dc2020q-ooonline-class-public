...
class UserRegistration(Resource):
    def post(self):
        ...
        result = db.session.execute(query, {'name': name, 'passwd': passwd})
        if not result:
            abort(500)
        result = list(result)
...
""",
    ...
    _smart_user = User(username='smart',
                        password='this-is-a-smart-password')

    db.session.add(_smart_user)
    db.session.commit()

    _correct_submission = Submission(user_id=_smart_user.id,
                                     assignment_id=1,
                                     filename="so_smart")

    db.session.add(_correct_submission)
    db.session.commit()

    _correct_result = SubmissionResult(submission_id=_correct_submission.id,
                                       result=True,
                                       message="You are so smart.")

    db.session.add(_correct_result)
    db.session.commit()
